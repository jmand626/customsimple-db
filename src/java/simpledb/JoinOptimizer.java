...
public class JoinOptimizer {
    ...
    public double estimateJoinCost(LogicalJoinNode j, int card1, int card2,
        ...
        if (j instanceof LogicalSubplanJoinNode) {
            ...
            // We just use the typical formula from lecture but the javadoc comment for this function also explains
            // that we need to include the cpu cost too. We just assume that the CPU cost is the cost product and we
            // count it for each pair of nodes
    ...
    public static int estimateTableJoinCardinality(Predicate.Op joinOp,
        ...
        int card = 1;
        // some code goes here
        // Handle the 5 different join types: equality PK both, PK left, PK right, PK none, and other non-equality joins
        if (joinOp == Predicate.Op.EQUALS) {
            if (t1pkey && t2pkey) {
                // "For equality joins, when one of the attributes is a primary key, the number of tuples produced by the
                // join cannot be larger than the cardinality of the non-primary key attribute." Although it does not
                // explicitly mentioned how to do this with two PKs, the same idea is obviously used here
                card = Math.min(card1, card2);
            } else if (t1pkey && !t2pkey) {
                // Same idea as with both, take the non pk card
                card = card2;
            } else if (!t1pkey && t2pkey) {
                // Flip who has an key
                card = card1;
            } else {
                // "For equality joins when there is no primary key, it's hard to say much about what the size of the
                // output is -- it could be the size of the product of the cardinalities of the tables (if both tables
                // have the same value for all tuples) -- or it could be 0. It's fine to make up a simple heuristic
                // (say, the size of the larger of the two tables)

                // We just use the heuristic
                card = Math.max(card1, card2);
            }
        } else{
            // Non equality joins. We just follow:
            // For range scans, it is similarly hard to say anything accurate about sizes. The size of the output should
            // be proportional to the sizes of the inputs. It is fine to assume that a fixed fraction of the
            // cross-product is emitted by range scans (say, 30%). In general, the cost of a range join should be larger
            // than the cost of a non-primary key equality join of two tables of the same size.
            card = (int) (0.3 * card1 * card2);
        }

        // This line was placed here already, but its ok since a negative cardinality doesnt make sense so an special
        // case is good.
